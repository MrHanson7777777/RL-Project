...
def build_agent_kwargs(algo, cfg):
    ...
    if algo == "actor_critic":
        agent_kwargs["actor_lr"] = cfg.get("actor_lr", cfg.get("lr", 1e-3))
        agent_kwargs["critic_lr"] = cfg.get("critic_lr", cfg.get("lr", 1e-3))
    if algo == "reinforce":
...
def main():
    ...
    if cfg.get("actor_lr") is not None or cfg.get("critic_lr") is not None:
